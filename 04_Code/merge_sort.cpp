...
void merge_sort(int *arr, int n)
{
  ...
  {
    if (n < 32)
    { // insertion sort for n smaller than 32
...
#pragma omp task if (size_b > 10000)  // make task if size_b > 10000, one task is enough
    merge_sort(arr + size_a, size_b); // recursive call
#pragma omp taskwait                  // wait until both subarrays are sorted
    if (n < 8192)
    { // allocate array on the stack for small n
