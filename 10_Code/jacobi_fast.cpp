...
int run(double * __restrict__ A, double * __restrict__ b, double * __restrict__ x, double * __restrict__ xtmp) {
  int itr;
  int row, col;
  double dot;
  double diff;
...
// #pragma omp parallel for num_threads(2)
    for (row = 0; row < N; row++) {
      ...
      auto new_x = (b[row] - (dot- A[row + row * N] * x[row])) / A[row + row * N];
      diff = new_x - x[row];
      sqdiff += diff * diff;
      x[row] = new_x;
    }
...
int main(int argc, char *argv[]) {
  ...
  double *xtmp = (double *)_mm_malloc(N * sizeof(double), 64);
...
//  N = 2;
//  double AA[4] ={16,3,7,-11};
//  double bb[2] = {11,13};
//  double xx[2] = {0,0};
//  int itrr = run(AA, bb, xx, xx);
